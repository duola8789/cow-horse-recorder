...
export type DailyStatus = 'recorded' | 'partial' | 'default' | 'missing' | 'rest' | 'leave' | 'today'
...
// 获取月度每日明细
export async function getMonthlyRecords(year: number, month: number): Promise<MonthlyRecordsResponse> {
  const res = await Taro.cloud.callFunction({
